  ...
  return (
    <Dialog.Root open={isOpen} onOpenChange={setIsOpen}>
      ...
      <Dialog.Portal>
        ...
        <Dialog.Content className="bg-white-100 data-[state=open]:animate-contentShow fixed top-1/2 left-1/2 max-h-[85vh] w-[346px] -translate-x-1/2 -translate-y-1/2  rounded-[5px] focus:outline-none lg:w-[576px]">
          <Command className="shadow-search-dialog border-white-border dark:border-white-border ">
            ...
            <Command.List className="bg-white-200 text-black-700 dark:bg-black-900 dark:text-white-200 px-1 py-2 lg:p-2 ">
              ...
              {!isLoading && (
                <Command.Empty className="p3-regular hover:bg-white-100 hover:dark:bg-black-700 flex gap-3 p-2.5 transition-colors hover:rounded lg:px-4 lg:py-2.5">
                  {q.length > 0
                    ? 'No results found.'
              ...
              {data?.map(({ id, title, type }) => (
                ...
                >
                  <Link
                    ...
                    className="p3-regular hover:bg-white-100 hover:dark:bg-black-700 line-clamp-1 flex gap-3 p-2.5 hover:rounded lg:px-4 lg:py-2.5"
                  >
                    {renderItemIcon(type)}
                    {title}
                  </Link>
